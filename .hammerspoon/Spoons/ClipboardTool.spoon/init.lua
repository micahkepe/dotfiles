...
function obj:_populateChooser(query)
	...
	for k, v in pairs(clipboard_history) do
		if v.type == "text" and (query == "" or v.content:lower():find(query)) then
			table.insert(
				menuData,
				{ text = string.sub(v.content, 0, obj.display_max_length), data = v.content, type = v.type }
			)
		elseif v.type == "image" then
			table.insert(
				menuData,
				{ text = "《Image data》", type = v.type, data = v.content, image = hs.image.imageFromURL(v.content) }
			)
		end
	end
	if #menuData == 0 then
	...
	if now > last_change then
		if (not self.honor_ignoredidentifiers) or self:shouldBeStored() then
			...
			if (current_clipboard == nil) and (pasteboard.readImage() ~= nil) then
				...
				self.logger.df(
					"Adding image (hashed) %s to clipboard history clipboard",
					hashfn(current_clipboard:encodeAsURLString())
				)
